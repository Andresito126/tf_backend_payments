...
describe('Collection', () => {
  describe('createForAcceptedOffer', () => {
    it('inicia en pending_delivery con los datos correctos', () => {
      ...
      expect(collection.getDeliveryCode()).toBe('code-abc');
      expect(collection.getFinalAmount()).toBeNull();
      expect(collection.isAmountConfirmed()).toBe(false);
    });
  });

  describe('scanDelivery', () => {
    it('pending_delivery → pending_weighing y registra timestamp', () => {
      const collection = buildCollection('pending_delivery');
      const now = new Date('2026-07-15T12:00:00Z');

      collection.scanDelivery(now);

      expect(collection.getStatus()).toBe('pending_weighing');
    ...
    it.each<CollectionStatus>([
      ...
      expect(() => collection.scanDelivery()).toThrow(InvalidCollectionTransitionException);
    });
  });

  describe('registerWeighing', () => {
    it('pending_weighing → pending_confirmation y calcula finalAmount', () => {
      const collection = buildCollection('pending_weighing');

      collection.registerWeighing(3.5, 12.5);
  ...
  describe('cancelByEstablishment', () => {
    it.each<CollectionStatus>(['pending_delivery', 'pending_weighing'])(
      'permitido desde %s',
      (status) => {
        const collection = buildCollection(status);
